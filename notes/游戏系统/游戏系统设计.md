...
在构建系统时，我始终遵循以下五大原则：
1. 数据驱动（Data-Driven）
2. 逻辑与表现分离（Logic-View Separation）
3. 组合优于继承（Composition over Inheritance）
 原则：少用深层继承树（BaseItem -> Equipment -> Weapon -> Sword），多用组件组合（Entity + ItemComponent + WeaponComponent）。
4. 接口隔离与解耦（Decoupling）
 原则：系统之间通过**事件（Event）或接口（Interface）**通讯，而不是直接引用。
5. 容错与防御性编程

### 三、构建方法论（The Methodology）
1. 阶段 1：技术方案评审（Technical Design Document）
 在写第一行代码前，先写文档。
 数据结构设计
 生命周期管理
 API 定义：对外提供什么接口？（如 Equip(itemId), Unequip(uid)）。
 性能预算

2. 阶段 2：核心逻辑实现（The "Model"）
先写纯代码逻辑，不碰引擎的UI和渲染接口。

3. 阶段 3：表现层接入（The "View"）
MVC/MVVM 模式：
UI 监听数据变化事件。
数据变了 -> UI 脏标记（Dirty Flag） -> 下一帧刷新 UI。